...
//const API_URL_ROL = "http://localhost:5000/api/rol";
//const API_URL_USUARIO = "http://localhost:5000/api/usuario";
...
const GestionarRolUsuario = () =>{
   ...
   //Estados para los datos
    ...
    //Filtrado cuando es un numero
    const rolFiltrado = listUsuarioRol.filter(usuarioRol => 
        busqueda === "" || usuarioRol.id_rol === Number(busqueda)
    );
    ...
    const obtenerDatosIniciales = async () => {
        try {
            ...
            const [resRolUsuario] = await Promise.all([
                fetch(API_URL),
            ]);
            
            const dataRolUsuario = await resRolUsuario.json();
    ...
    const eligeRegistro = (usuarioRol) =>{
        setDatos({
            ...usuarioRol,
        });
        setMostrarForm(true);
    ...
    const cambioInput = (e) =>{
        ...
        if (name === "id_rol" || name === "id_usuario") {
    ...
    return(
        ...
        <header className="text-center mt-3">
            ...
            <h6>Elige un rol de usuario para Editar</h6>
        </header>
            ...
            <div className="d-flex flex-nowrap justify-content-center gap-4" >
                ...
                <div id="wrapperForm" className="card shadow p-4" style={{ flexShrink: 0, marginBottom: "30px" }}>
                    <form id="formTipoDocumento2" onSubmit={registrar}>
                        <label htmlFor="id_rol" className="d-block mb-3">ID Rol
                        <input type="text" name="id_rol" id="id_rol" className="form-control border-primary-subtle" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={datos.id_rol} onChange={cambioInput} required/>
                        </label>

                        <label htmlFor="id_usuario" className="d-block mb-3">ID de Usuario
                        <input type="text" name="id_usuario" id="id_usuario" className="form-control border-primary-subtle" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={datos.id_usuario} onChange={cambioInput} required/>
                        </label>
                ...
                <section id="tablaTipoDocumento" className="flex-grow-1" style={{ marginRight: "10px", minWidth: 0, maxWidth: "30vw", alignSelf: "flex-start" }}>
                    <div className="shadow-sm mb-0 bg-default p-1" >
                        <div className="d-flex w-100 gap-3">
                            <input
                                type="text" className="form-control form-control-lg" placeholder="Buscar por ID de Usuario..." 
                        ...
                        <table id="tpaciente" className="table table-striped table-hover shadow-sm align-middle mb-0" style={{ border: "3px solid rgb(255, 255, 255)", borderRadius: "10px", overflow: "hidden" }} >
                            <thead className="table-dark text-center align-middle">
                                <tr>
                                <th scope="col">ID Rold</th>
                                <th scope="col">ID de Usuario</th>
                                </tr>
                            </thead>
                            <tbody className="text-center" style={{ cursor: "pointer" }}>
                                {rolFiltrado.length > 0 ? (
                                    rolFiltrado.map((usuarioRol) => (
                                        <tr key={usuarioRol.id_rol} onClick={() => eligeRegistro(usuarioRol)}>
                                            <th scope="row">{usuarioRol.id_rol}</th>
