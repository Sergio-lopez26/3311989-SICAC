...
const GestionarRolUsuario = () =>{
   ...
   //Estados para los datos
    ...
    const modalDeseaRegistrar = (id_rol, id_usuario) =>{ //parametros referentes a los campos, para almacenar sus valores
        setDatosIniciales({ id_rol, id_usuario});

        const modalDReg = document.getElementById('deseaRegistrar');
        let modalBootstrap = bootstrap.Modal.getInstance(modalDReg) || new bootstrap.Modal(modalDReg);
        modalBootstrap.show();
    };

    const registrarRolDirecto = async () => {
        const { id_rol, id_usuario } = datosIniciales;
        if(!id_rol || !id_usuario) return;

        const modalDesea = document.getElementById('deseaRegistrar');
        bootstrap.Modal.getInstance(modalDesea)?.hide(); //Esto para ocultar el modalDeseaRegistrar en caso se haya orpimido si
    ...
    const modalDeseaEliminar = (id_rol, id_usuario) =>{ //parametros referentes a los campos, para almacenar sus valores
        setDatosIniciales({ id_rol, id_usuario});
    ...
    const eliminarRolDirecto = async () => {
        const { id_rol, id_usuario } = datosIniciales;
        if(!id_rol || !id_usuario) return;

        const modalDesea = document.getElementById('deseaEliminar');
        bootstrap.Modal.getInstance(modalDesea)?.hide(); //Esto para ocultar el modalDeseaEliminar en caso se haya orpimido si
    ...
    const cancelar = () => {
        const modalReg = bootstrap.Modal.getInstance(document.getElementById('deseaRegistrar'));
        const modalEli = bootstrap.Modal.getInstance(document.getElementById('deseaEliminar'));
        ...
        document.querySelector('.modal-backdrop')?.remove();
        document.body.style.overflow = '';
        obtenerRolUsuario(); //Se sincroniza y restauran los select visualmente
    };
    ...
    return(
        ...
        <div className="container-fluid mt-4 px-4 d-flex justify-content-center rol_usuario"> 
                                    ...
                                    <tr key={`${usuarioRol.id_rol}-${usuarioRol.id_usuario}`}>
                                        ...
                                        <td>
                                            ...
                                            <select 
                                                ...
                                                onChange={(e) => modalDeseaRegistrar(e.target.value, usuarioRol.id_usuario)}
                                            ...
                                            {usuarioRol.id_rol === 1 && usuarioRol.id_usuario !== 1 && (
                                                ...
                                                onClick={() => modalDeseaEliminar(usuarioRol.id_rol, usuarioRol.id_usuario)}>
                                                🗑️
                                                </button>
                                            )}
                                            </div>
            ...
            {/* MODALES */}
            <div className="modal fade" id="deseaRegistrar" tabIndex="-1" aria-hidden="true">
                <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                    <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                    
                        <h4 className="text-success fw-bold mb-3">Asingar Nuevo Rol</h4>
                        
                        <p className="text-muted mb-4">Estimado Administrador. ¿Desea asignar este rol al usuario?</p>
                        
                        <div className="d-flex w-100 gap-3 mt-4">
                            <button type="button" className="btn btn-primary w-50 py-2" id="btnSalir" style={{borderRadius: "12px"}} 
                            onClick={registrarRolDirecto}>
                                Sí
                            </button>
                            <button type="button" className="btn btn-secondary w-50 py-2" id="btnCerrar" style={{borderRadius: "12px"}} 
                            onClick={cancelar}>
                                No
                            </button>
                        </div>
            ...
            <div className="modal fade" id="eliminar" tabIndex="-1" aria-hidden="true" data-bs-backdrop="static">
                <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                    <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                        ...
                        <p className="text-muted mb-4">El rol asignado al usuario ha sido eliminado en el sistema exitosamente.</p>
