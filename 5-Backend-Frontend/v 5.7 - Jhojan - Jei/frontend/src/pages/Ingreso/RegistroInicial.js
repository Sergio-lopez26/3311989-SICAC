...
function RegistroInicial({propsRegistro}){ //Para que actue como props tambien y otra pagina la pueda llamar
    ...
    const modalRegistro = () => {
        ...
        if(modalExiste){
            ...
            if (propsRegistro) {
            //Si se abrió este modal desde la pantalla de RegistroMedico, se devuelve los datos allá y frena la navegación
            propsRegistro({
                id: idUsuarioRef.current,
                email: emailCapturado
            });
            } else {//osea que se está registrando el usuario desde esta misma pagina
        ...
        try{
            ...
            if(res.ok){
                ...
                if (propsRegistro) {
                    ...
                    //Mandamos los datos a la otra pagina directamente
                    propsRegistro({
                        id: idUsuarioRef.current,
                        email: emailCapturado
                    });
    ...
    return(
        <>
        <div className={propsRegistro ? "" : "cambio-page"}> {/* Si se le llama desde otra pagina, o si es nativo (ahi si aplica el css) */}
            {!propsRegistro &&( //Solo se mostrara con la pagina nativa lo que vaya dentro del &&()
            <header>
            ...
            <form id="formRegistro" onSubmit={Registrarse}>           
                ...
                <div className="d-flex w-100 gap-3 mt-4">
                    ...
                    <button className="btn btn-secondary flex-fill py-2" type="button" onClick={cancelar} data-bs-dismiss={propsRegistro ? "modal" : ""}>Cancelar</button>
                    {/* data-bs-dismiss={propsRegistro ? "modal" : ""} -> Sirve para que si es modal, al dar en canelar se cierre sin hacer mas */}
                </div>
                {!propsRegistro && (//Lo mismo, o aparece vacio este div, si se llama como modal, o aparece con el Link si es la misma pagina
