...
const API_URL_DIAGNOSTICO = "http://localhost:5000/api/diagnostico/cita/"; // Ruta correcta según tu backend
    ...
    const obtenerHistorialYDiagnosticos = async () => {
      try {
        ...
        if (resCitas.ok) {
            ...
            const citasAtendidas = citas.filter(cita => cita.estado_cita === 'programada');
