...
const API_URL = "http://localhost:5000/api/tipo_cita";

const GestionarTiposCita = () =>{

    const [mostrarForm, setMostrarForm] = useState(false);
    const [mostrarForm2, setMostrarForm2] = useState(false);
    const [listTipoCita, setListTipoCita] = useState([]);
    ...
    const tipoCitaFiltrado = listTipoCita.filter((tc) => {
        const ttipo = (tc.tipo_cita || "").toLowerCase();
    ...
    const registrar = async (e) => {
        ...
        try{
            ...
            console.error("Error al registrar el nuevo tipo de cita:", error);
    ...
    const eliminar = async () => {
        if (!window.confirm("¿Está seguro de eliminar este tipo de cita?")) return;
        try {
    ...
    return(
        ...
        <header className="text-center mt-3">
            <h1 style={{ color: "#103e6e", fontWeight: "bolder" }}>Tipos de Cita</h1>
            <h6>Elige un tipo de cita para editar</h6>
        </header>
            ...
            <div className="d-flex flex-nowrap justify-content-center gap-4">
                ...
                <div id="wrapperForm" className="card shadow p-4" style={{ flexShrink: 0, marginBottom: "30px", maxHeight: "35vh" }}>
                    <form onSubmit={actualizar}>
                        <label htmlFor="tipo_cita" className="d-block mb-3">Tipo de cita
                        <input type="text" name="tipo_cita" id="tipo_cita" className="form-control border-primary-subtle" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={datos.tipo_cita} onChange={cambioInput} required/>
                        </label>
                ...
                <section className="flex-grow-1" style={{ marginRight: "10px", minWidth: 0, maxWidth: "30vw", alignSelf: "flex-start" }}>
                    <div className="shadow-sm mb-0 bg-default p-1">
                        <div className="d-flex w-100 gap-3">
                            <input
                                type="text" className="form-control form-control-lg" placeholder="Buscar tipo de cita..."
                        ...
                        <table className="table table-striped table-hover shadow-sm align-middle mb-0" style={{ border: "3px solid rgb(255, 255, 255)", borderRadius: "10px", overflow: "hidden" }}>
                            ...
                            <tbody className="text-center" style={{ cursor: "pointer" }}>
                                {tipoCitaFiltrado.length > 0 ? (
                                    tipoCitaFiltrado.map((tc) => (
                                        <tr key={tc.id} onClick={() => eligeRegistro(tc)}>
                                    ...
                                    <tr>
                                        <td colSpan="2" className="text-center text-muted py-4">
                                            No se encontró tipo de cita consultado.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                    <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                        ...
                        <p className="text-muted mb-4">El tipo de cita ha sido actualizado en el sistema exitosamente.</p>
                ...
                <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                    <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                        ...
                        <p className="text-muted mb-4">El tipo de cita ha sido registrado en el sistema exitosamente.</p>
...
export default GestionarTiposCita;
