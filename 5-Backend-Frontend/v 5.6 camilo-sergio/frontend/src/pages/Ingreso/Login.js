...
const Login = () =>{
    ...
    const [rolesDisponibles, setRolesDisponibles] = useState([]); //Se almacenaran los roles que tenga el usuario asignados
    ...
    const modalIngreso = () => {
        ...
        //Adicional de que va segun el rol por donde el usuario desee loguearse, salvo si se tiene un solo rol, pues queda automático
        if(sesion) {
            if(sesion.rol === "Administrador"){
                navegar('/Menu_Administrador');
            }else if(sesion.rol === "Odontologo"){
                navegar('/Menu_Medico');
            }else if(sesion.rol === "Paciente"){
                navegar('/Menu_Paciente');
            }else {
                navegar('/');
            }
        }else{
    ...
    const escogerRol = (rolElegido) => {
        const sesionElegida = {
            id: datosUsuarioLogueado.id,
            email: datosUsuarioLogueado.email,
            nombre_completo: datosUsuarioLogueado.nombre_completo,
            rol: rolElegido
        };

        ///Con esto se guarda los datos del usuario en el navegador
        localStorage.setItem("usuario", JSON.stringify(sesionElegida));

        const modalIng = document.getElementById('ingreso');
        let modalBootstrap = bootstrap.Modal.getInstance(modalIng) || new bootstrap.Modal(modalIng);
        modalBootstrap.show();

        const modalERol = bootstrap.Modal.getInstance(document.getElementById('elegirRol'));
        
        if (modalERol) modalERol.hide();

        setRolesDisponibles([]);
        setDatosUsuarioLogueado(null);
    };
    ...
    const procesarLogin = async (e) => {
        ...
        try {
            ...
            if(res.ok){
                ...
                //Si el usuario tiene mas de 1 rol, se produce el desvío desde el backend
                if (data.ElegirRol) {
                    //Se guarda los roles en un estado para que luego se pueda elegir uno de estos
                    setRolesDisponibles(data.usuarioReal.roles);
                    setDatosUsuarioLogueado(data.usuarioReal);

                    const modalElegir = document.getElementById('elegirRol');
                    let modalBootstrap = bootstrap.Modal.getInstance(modalElegir) || new bootstrap.Modal(modalElegir);
                    modalBootstrap.show();
                    return;
                }
    ...
    return(
        <div className="cambio-page">
            ...
            <div className="modal fade" id="elegirRol" tabIndex="-1" aria-hidden="true" data-bs-backdrop="static">
                <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                    <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                    
                        <h4 className="text-dark fw-bold mb-3">Seleccionar Rol de Ingreso</h4>
                        <p className="text-dark small mb-4">Estimado(a) <strong>{datosUsuarioLogueado?.nombre_completo}</strong>.<br /> 
                            Su cuenta posee múltiples roles de acceso. ¿Cómo desea ingresar hoy?
                        </p>
                        <div className="d-flex align-items-center justify-content-center gap-3 mt-4">
                            {rolesDisponibles.map((rol) => (
                                <button key={rol} type="button" className={`btn w-100 py-2 ${
                                        rol === 'Administrador' ? 'btn-danger' : 
                                        rol === 'Odontologo' ? 'btn-primary' : 'btn-success'
                                    }`}
                                    style={{ border: "none" }} onClick={() => escogerRol(rol)}
                                >
                                    {rol === 'Odontologo' ? 'Médico' : rol} {/* Aquí es para que en vez de Odontologo se muestre Medico, ya que como rol tenemos odontologo */}
                                </button>
                            ))}
                        </div>
                        <button type="button" className="btn btn-light py-2 mt-1 small text-muted" onClick={cancelar}>
                                Cancelar
                        </button>
                    </div>
                </div>
