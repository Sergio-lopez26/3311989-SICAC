...
// Ajuste: estados clínicos usados en el odontograma y su color representativo para pintar cada diente.
const ESTADOS = {
    "Sano": "#8fce8f",
    "Cariado": "#f28b82",
    "Obturado": "#8ab4f8",
...
// Ajuste: normalización de los estados para aceptar variantes como "Sana"/"Sano" y evitar que el diente quede sin color aunque ya exista en la BD.
const normalizarEstado = (estado) => {
    ...
    const mapa = {
        ...
        sellante: "Sellante",
        "extraccion indicada": "Extracción Indicada",
        "extracción indicada": "Extracción Indicada",
        "extraccion_indicada": "Extracción Indicada",
        "extracción_indicada": "Extracción Indicada"
    };

    const clave = String(estado).trim().toLowerCase();
    return mapa[clave] || String(estado).trim() || "Sano";
};
...
const OdontogramaCore = ({ Navbar, rutaVolver, soloLectura = false }) => {
    ...
    const colorDiente = (fdi) => {
        ...
        const estadoActual = normalizarEstado(pieza.estado_actual || pieza.estado_inicial);
        return ESTADOS[estadoActual] || "#ffffff";
    };
    ...
    const seleccionarDiente = (fdi) => {
        ...
        if (pieza) {
            setDatosForm({
                ...pieza,
                estado_inicial: normalizarEstado(pieza.estado_inicial),
                estado_actual: normalizarEstado(pieza.estado_actual || pieza.estado_inicial)
            });
        } else {
            setDatosForm({
                id: "",
