...
const GestionarRoles = () =>{
    ...
    const [listRol, setListRol] = useState([]); //Almacenara los datos de rol
    ...
    const rolFiltrado = listRol.filter((rol) => {
        const trol = (rol.rol).toLowerCase();

        return (
            trol.includes(busqueda.toLowerCase())
        );
    });
    ...
    const registrar = async (e) => {
        ...
        try{
            ...
            console.error("Error al registrar el nuevo rol:", error);
    ...
    const modalDeseaEliminar = (id) =>{
        const modalDEli = document.getElementById('deseaEliminar');
        let modalBootstrap = bootstrap.Modal.getInstance(modalDEli) || new bootstrap.Modal(modalDEli);
        modalBootstrap.show();
    };
    ...
    const eliminarRol = async () => {
        ...
        const modalDesea = document.getElementById('deseaEliminar');
        bootstrap.Modal.getInstance(modalDesea)?.hide(); //Esto para ocultar el modalDeseaEliminar en caso se haya oprimido si
        
        try {
            const eliminarR = await fetch(`${API_URL}/${id}`, {
                method: "DELETE"
            });
            ...
            if (eliminarR.ok) {
                ...
                //Para refrescar la tabla con la data actualizada en la BD tras la eliminación del Rol
                await obtenerDatosIniciales();
            }else{
                const errorData = await eliminarR.json();
                alert(`Error: ${errorData.error}`);
            }
        } catch (error) {
    ...
    const cancelaEliminar = () =>{
        const modalDesea = document.getElementById('deseaEliminar');
        bootstrap.Modal.getInstance(modalDesea)?.hide();
    }
    ...
    return(
        ...
        <header className="text-center mt-3">
            <h1 style={{ color: "#103e6e", fontWeight: "bolder" }}>Roles</h1>
            <h6>Elige un rol para Editar</h6>
        </header>
            ...
            <div className="d-flex flex-nowrap justify-content-center gap-4" >
                ...
                <div id="wrapperForm" className="card shadow p-4" style={{ flexShrink: 0, marginBottom: "30px", maxHeight: "30vh" }}>
                    <form id="formTipoDocumento" onSubmit={actualizar}>
                        <label htmlFor="rol" className="d-block mb-3">Rol
                        <input type="text" name="rol" id="rol" className="form-control border-primary-subtle" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={datos.rol} onChange={cambioInput} required/>
                        </label>
                        ...
                        <div className="d-flex w-100 gap-3">
                            ...
                            <button id="btnEliminar" type="button" className="btn btn-danger flex-fill py-2" style={{ borderRadius: "12px" }} onClick={modalDeseaEliminar}>Eliminar</button>
                            <button id="btnCancelar" type="button" className="btn btn-secondary flex-fill py-2" style={{ borderRadius: "12px" }} onClick={cancelar}>Cancelar</button>
                        </div>
                    </form>
                </div>
                )}
                ...
                <section id="tablaRol" className="flex-grow-1" style={{ marginRight: "10px", minWidth: 0, maxWidth: "30vw", alignSelf: "flex-start" }}>
                    <div className="shadow-sm mb-0 bg-default p-1" >
                        <div className="d-flex w-100 gap-3">
                            <input
                                type="text" className="form-control form-control-lg" placeholder="Buscar rol..." 
                        ...
                        <table id="trol" className="table table-striped table-hover shadow-sm align-middle mb-0" style={{ border: "3px solid rgb(255, 255, 255)", borderRadius: "10px", overflow: "hidden" }} >
                            ...
                            <tbody className="text-center" style={{ cursor: "pointer" }}>
                                {rolFiltrado.length > 0 ? (
                                    rolFiltrado.map((rol) => (
                                        <tr key={rol.id} onClick={() => eligeRegistro(rol)}>
                ...
                <div className="modal fade" id="actualiza" tabIndex="-1" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                        <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                            <h4 className="text-success fw-bold mb-3">¡Rol Actualizado!</h4>
                            <p className="text-muted mb-4">El rol ha sido actualizado en el sistema exitosamente.</p>
                ...
                <div className="modal fade" id="registro" tabIndex="-1" aria-hidden="true" data-bs-backdrop="static">
                    <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                        <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                            ...
                            <p className="text-muted mb-4">El rol ha sido registrado en el sistema exitosamente.</p>
                ...
                <div className="modal fade" id="deseaEliminar" tabIndex="-1" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                        <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                        
                            <h4 className="text-danger fw-bold mb-3">Eliminar Asignación de Rol</h4>
                ...
                <div className="modal fade" id="eliminar" tabIndex="-1" aria-hidden="true" data-bs-backdrop="static">
                    <div className="modal-dialog modal-dialog-centered" style={{maxWidth: "380px"}}>
                        <div className="modal-content text-center border-0 p-4" style={{borderRadius: "16px"}}>
                            ...
                            <p className="text-muted mb-4">El tipo de documento ha sido eliminado en el sistema exitosamente.</p>
