...
const funcionValida = (req, res, next) => {
    ...
    if(!validaLetras.test(nombres)){
        return res.status(400).json({error: "Los nombres solo deben contener letras y tildes"})
    }
    if(!validaLetras.test(apellidos)){
        return res.status(400).json({error: "Los apellidos solo deben contener letras y tildes"})
    }
...
//Obtener registro de un medico -> Perfil
router.get('/:id', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM medico WHERE id_usuario = ?', [req.params.id]);
...
router.post('/', funcionValida, async (req, res) => {
    try {
        ...
        //Para evitar que un usuario-paciente sea tambien usuario-medico
        const [existPaciente] = await pool.query(
            'SELECT id FROM paciente WHERE id_usuario = ?',
            [id_usuario]
        );

        if(existPaciente.length > 0){
            return res.status(400).json({
                error: 'El Usuario ya existe como Paciente'
            })
        }
        ...
        //Con esto nos curamos en salud, ya que automaticamente, el médico registrado adquiere el rol de medico en la tabla rol_usuario
        await pool.query(
...
router.put('/:id', funcionValida, funcionValEmail, async (req, res) => {
    try {
        ...
        const usuarioMedico = currentMedico[0]; //Se extrae la fila a la que pertenece el registro

        let hashedPass = usuarioMedico.password; //Se guarda el password sin cambio
